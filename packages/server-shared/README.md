...
##### 1. Urgent combat (hostile encounter)

- Sub-agent sends `spark:notify` (kind=alarm, urgency=immediate, payload hp/location/gear, destinations=["character"]).
- Character `spark:emit` working ("Seen it").
- Character issues `spark:command` with interrupt=force and options (retreat vs push).
- Sub-agent `spark:emit` working ("Taking cover; healing") then done/blocked as it executes.
- Optional `context:update` for summary/memory.

##### 2. Prep plan (boss fight)

- Discord/user intent triggers character `spark:command` to sub-agent (intent=plan, interrupt=soft, steps gather supplies/gear, fallback).
- Optional `context:update` with tips (lane='game').
- Sub-agent streams `spark:emit` progress.
- If ambushed, sub-agent raises new `spark:notify` (alarm/immediate) to preempt.
- Character revises with another `spark:command`.
- Completion via `spark:emit` done + summary note.

##### 3. Routine nudge

- Sub-agent signals low resources via `spark:notify` (kind=reminder, urgency=soon, destinations=["character"]).
- Character defers to next tick and sends `spark:command` (interrupt=soft, intent=plan: "gather supplies nearby").
- Sub-agent `spark:emit` queued/working then done.
...
- Character receives a user goal (e.g., fortify base) and issues an initial `spark:command` to sub-agent (interrupt=soft, intent=plan, steps to gather materials) so the agent keeps working.
...
- If an urgent event occurs during prep, sub-agent raises `spark:notify` (alarm) → character responds with a short `spark:emit` working and a `spark:command` (interrupt=force) to handle it (e.g., retreat, defend).
...
- Sub-agent streams `spark:emit` progress; when complete, character summarizes via `spark:emit` or `context:update` for memory.
